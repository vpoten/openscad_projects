...
difference(){
	...
	translate([0,0,-0.5]) cylinder(r=7,h=plateThick+1,$fn=20);
	...
	for ( i = [-1,1]) for(j=[-1:1]) translate([i*31,j*30,-0.5]) cylinder(r=m3Rad, h=plateThick+1,$fn=20);
	...
	//rear holes
	for(j = [-1,1]){
	translate([j*25,-armDist/2-5,-0.5]) cylinder(r=m3Rad,h=plateThick+1,$fn=20);
	translate([j*16,-armDist/2-5,-0.5]) cylinder(r=m3Rad,h=plateThick+1,$fn=20);
	}

	
	if(top==false){
		translate([0,-armDist/2+10,-0.5]) roundedRect([15,8,plateThick+1], 3);
		for ( j = [-1,1]){ //rear holes
		translate([j*24,-armDist/2+12,-0.5]){ roundedRect([2,20,plateThick+1], 2);}
		}
	}
	else{
		// fpv antenna holes
		translate([0,-armDist/2-4,-0.5]) cylinder(r=3.7,h=plateThick+1,$fn=20);
		for ( j = [-1,1]) translate([j*36,-armDist/2+16,-0.5]) cylinder(r=3.7,h=plateThick+1,$fn=20);
		for(i=[-4:1]) translate([0,(-armDist/2+30)-i*20,-0.5]) roundedRect([22,5,plateThick+1], 3.2);
		for(i=[-1,1]) for ( j = [-1:1]){ //rear holes
		translate([i*23,(-armDist/2+62)+j*40,-0.5]){ roundedRect([2,27,plateThick+1], 1.2);}
		}
	}
...
module bottomPlate(xtra=0.25){
	...
	variableCross(45.5, 29.5, thick=plateThick+xtra);
	...
	for ( i = [-1,1]) { for ( j = [-1,1]) { //ESC holes
		translate([j*29,i*50,-0.5]) roundedRect([2,20,plateThick+xtra+1], 1.2);
		translate([j*18.5,i*50,-0.5]) roundedRect([7,20,plateThick+xtra+1], 2);
		translate([0,i*45,-0.5]) roundedRect([11,30,plateThick+xtra+1], 2);
	}}
	}//translate
...
color("SkyBlue", 0.8) translate([0,-armDist/2+35,15]) rotate([0,0,90]) battery();
